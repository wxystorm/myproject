...
Completed extension: **6.4 Memory**. The agent now exposes `memory_read` and `memory_write` tools. They persist project-level notes in `.agent/memory.md`; `memory_write` creates `.agent` when needed and supports append mode by default or replace mode via `"mode": "replace"`, while `memory_read` returns the saved memory or an empty-memory message.
