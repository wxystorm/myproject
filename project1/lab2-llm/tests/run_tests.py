#!/usr/bin/env python3
"""Unified test runner for the LLM HTTP client lab (public tests only).

Usage:
    python3 tests/run_tests.py [--bin PATH] [--phase {a,b}] [--json]
"""
...
_PHASE_LABELS = {"a": "Phase A", "b": "Phase B"}
_SEPARATOR = "\u2501" * 50  # ━
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run LLM lab public tests.")
    parser.add_argument(
        "--bin", default="./llm", help="Path to the LLM client binary (default: ./llm)"
    )
    parser.add_argument("--phase", choices=["a", "b"], help="Run only the given phase")
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    results = run_cases(binary, cases)
